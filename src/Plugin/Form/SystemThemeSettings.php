 ...
 *
 * @AeonForm("system_theme_settings")
 */
class SystemThemeSettings extends FormBase implements FormInterface {
  ...
  public function alterFormElement(Element $form, FormStateInterface $form_state, $form_id = NULL) {
    ...
    // Creates the necessary groups (vertical tabs) for a Aeon based theme.
   ...
   *
   * @param \Drupal\aeon\Utility\Element $form
  ...
  protected function createGroups(Element $form, FormStateInterface $form_state) {
    ...
    $form['aeon'] = [
      '#type' => 'vertical_tabs',
      '#attached' => ['library' => ['aeon/theme-settings']],
      '#prefix' => '<h2><small>' . t('Aeon Settings') . '</small></h2>',
      '#weight' => -10,
    ];
    $groups = [
      'general' => t('General'),
   ...
   *   The current state of the form.
   *
   * @return \Drupal\aeon\Theme|false
   *   The currently selected theme object or FALSE if not a Aeon theme.
   */
  public static function getTheme(Element $form, FormStateInterface $form_state) {
    $build_info = $form_state->getBuildInfo();
    $theme = isset($build_info['args'][0]) ? Aeon::getTheme($build_info['args'][0]) : FALSE;

    // Do not continue if the theme is not Aeon specific.
    if (!$theme || !$theme->isAeon()) {
      unset($form['#submit'][0]);
