...
use Drupal\aeon\Plugin\PrerenderManager;
use Drupal\aeon\Plugin\ProcessManager;
    ...
    foreach (array_keys($types) as $type) {
      ...
      if ($type !== 'form') {
        ...
        foreach ($definitions as $definition) {
          Aeon::addCallback($element['#process'], [$definition['class'], 'process'], $definition['replace'], $definition['action']);
        }
        ...
        foreach ($pre_render_manager->getDefinitionsLike($regex) as $definition) {
          Aeon::addCallback($element['#pre_render'], [$definition['class'], 'preRender'], $definition['replace'], $definition['action']);
        }
      }
    }
  }
